...
{
    int rc;
    // 커널에게 socket(시스템 콜)을 이용하여 소켓 생성을 요청
    // 실패 시 -1 return
...
{
	int rc;
	// 1. 커널에 소켓과 주소의 바인딩을 요청한다.
	if(rc = bind(sockfd, addr, addrlen) < 0)
	// 2. 실패 시(-1 반환) 에러 메시지를 출력하고 프로그램을 종료한다.
	unix_error("Bind error");
	// 3. 성공하면 함수는 그냥 리턴된다.(return값 없음)
}

void Listen(int sockfd, int backlog)
{
	int rc;
	// 1. 커널에 해당 소켓을 리스닝 상태로 전환하라고 요청.
...
void Accept(int listenfd, struct sockaddr *addr, int *addrlen)
{
	int rc;
	// 1. 커널에 연결 수락을 요청한다. 연결 요청이 없으면 멈춰 기다린다.(블로킹 Blocking)
	if(rc = accept(listenfd, addr, addrlen) < 0)
	// 2. 실패 시(-1 반환) 에러 메시지를 출력하고 프로그램을 종료한다.
	unix_error("Accept error");
	// 3. 성공 시, 새로 생성된 연결 소켓의 파일 디스크립터를 반환한다.
	return rc;
}
...
void Connect(int sockfd, struct sockaddr *serv_addr, int addrlen) 
{
    ...
    // 2. 연결이 실패하면 (-1을 반환) 에러 메시지를 출력하고 프로그램을 종료한다.
	unix_error("Connect error");
    // 3. 성공하면 함수는 그냥 리턴된다(리턴 값이 없음)
}
...
{
	int rc;
	// 1. getaddrinfo 시스템 콜을 호출한다.
	if(rc = getaddrinfo(node, service, hints, res) != 0)
	// 2. 실패 시(0이 아닌 값 반환) gai_error를 호출한다
	gai_error(rc, "Getaddrinfo");
	// 3. 성공하면 res가 가리키는 곳에 결과 목록이 저장된다.
}
/* $end getaddrinfo */
...
int Open_clientfd(char *hostname, char *port) 
{
    ...
    if ((rc = open_clieentfd(hostname, port)) < 0) 
